...
if (config['experimental-cleaning'] == "true") {
    ...
    if (clean_combine == "cleaning") {
        ...
        require('child_process').execSync('node config_setter.js --clean=true --asked=true --remove=' + set_clean)
        wait(650)
        console.log("cleaning")
        require('child_process').execSync('node cleaner.js')
        wait(1500)
        exit
    }
if (clean_combine == "both") {
    ...
    require('child_process').execSync('node combiner.js')
    wait(3500)
    exit
}
...
if (clean_combine == "reset") {
    ...
    require('child_process').execSync('node config_setter.js --reset=true')
     
}
if (clean_combine == "debug") {
    console.log("debug has been enabled")
    require('child_process').execSync('node config_setter.js --debug=true')
}
}
else {
